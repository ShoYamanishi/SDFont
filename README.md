...
Another idea is to perform the dead-reckoning for a bigger glyph bitmap and then down-sample the values, by taking the average of each regular rectangular region. It may work if the error accummulated in each rectangular region is probabilistically independent, which may not be likely, as the errors seem to be radially propagated outwards.
